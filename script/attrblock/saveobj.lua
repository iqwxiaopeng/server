...
local function add_saveobj(obj)
	...
	__saveobjs[__id] = obj
	obj.__id = __id
	g_serverinfo:logger("saveobj",string.format("add_saveobj %s",obj:uniqueflag()))
end

local function del_saveobj(obj)
	g_serverinfo:logger("saveobj",string.format(" del_saveobj %s",obj:uniqueflag()))
	local id = obj.__id
	__saveobjs[id] = nil
	obj:nowsave()
end
...
local function onerror(err)
	msg = string.format("sknerror [ERROR] %s %s\n",os.date("%Y-%m-%d %H:%M:%S"),err)
		.. debug.traceback()
	g_serverinfo:logger("saveobj",msg)
	skynet.error(msg)
end
...
local function ontimer(id)
	...
	if obj then
		...
		g_serverinfo:logger("saveobj",string.format("%s ontimer",flag))
		g_gametimer:timeout(functor(ontimer,obj.__id),SAVE_DELAY,flag)
		obj:nowsave()
	end
end
...
local function starttimer(obj)
	local flag = obj:uniqueflag()
	g_serverinfo:logger("saveobj",string.format("%s starttimer",flag))
	g_gametimer:timeout(functor(ontimer,obj.__id),SAVE_DELAY,flag)
end
...
function csaveobj:autosave()
	...
	g_serverinfo:logger("saveobj",string.format(" %s autosave",self:uniqueflag()))
...
function csaveobj:merge(obj)
	...
	g_serverinfo:logger("saveobj",string.format("%s merge %s",self:uniqueflag(),obj:uniqueflag()))
	self.mergelist[id] = obj
end

function csaveobj:oncesave()
	assert(self.saveflag ~= "autosave","oncesave conflict with autosave")
	g_serverinfo:logger("saveobj",string.format("%s oncesave",self:uniqueflag()))
	self.saveflag = "oncesave"
end

function csaveobj:savetodatabase()
	g_serverinfo:logger("saveobj",string.format("%s savetodatabase",self:uniqueflag()))
...
function csaveobj:loadfromdatabase()
	g_serverinfo:logger("saveobj",string.format("%s loadfromdatabase",self:uniqueflag()))
...
function csaveobj:nowsave()
	if self.saveflag == "oncesave" or self.saveflag == "autosave" then
		pcall(function ()
			self:savedatabase()
			g_serverinfo:logger("saveobj",string.format("%s mergelist: %s",self:uniqueflag(),isempty(self.mergelist)))
...
function csaveobj:clearsaveflag()
	g_serverinfo:logger("saveobj",string.format("%s clearsaveflag",self:uniqueflag()))
	self.saveflag = false
end
